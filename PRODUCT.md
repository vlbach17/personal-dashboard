...
- Guides: a Kingshot (mobile game) guide database stored in Cloudflare D1 (not synced from files), with a simple in-app editor and filter/sort by category or tag.
...
- Edit/delete affordance is a single edit icon, used identically across every list (habits, captures, projects, guides) — no separate delete icon/swipe pattern.
...
No existing logo, name, or brand identity beyond the working title "Personal Dashboard" and domain `vanessab.design`. Design system direction: Material Design 3 (M3) — structure (spacing, elevation, color roles, motion) **and** the stock M3 baseline color palette and type (Roboto) for now, taken as-is rather than customized. This is a deliberate standing choice, confirmed 2026-09-11: build straight Material 3 first, revisit a custom palette/personality later. Craft bar: Google's own M3 apps (Calendar, Tasks, Keep) — canon done well, not a token library applied loosely. PWA icon and short name are still an explicitly open, undecided design decision — do not invent one without asking.
